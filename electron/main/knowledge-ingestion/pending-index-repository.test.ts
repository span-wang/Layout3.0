...
test('PH3-13C4 metadata builder 受控构造 pending、active 与 superseded 状态', async () => {
  await withRegistry(({ store }) => {
    createProcessingVersion(store, {
      ...
      metadata: {
        ...
        curriculumYear: 2024,
        status: 'archived',
        index_generation: '伪造代次',
        canonical_id: '伪造身份',
      },
    });
    const version = store.getMaterialVersion('ver-metadata');
    const metadata = buildLayout3PendingMetadata(
      version,
      'generation-2026-07',
    );
    const activeMetadata = buildLayout3ActiveMetadata(version, 'generation-2026-07');
    const supersededMetadata = buildLayout3SupersededMetadata(version, 'generation-2026-07');
    assert.equal(metadata.metadata_schema, 'layout3_ingestion_v1');
    assert.equal(metadata.status, 'pending');
    assert.equal(activeMetadata.status, 'active');
    assert.equal(supersededMetadata.status, 'superseded');
    for (const controlledMetadata of [metadata, activeMetadata, supersededMetadata]) {
      assert.equal(controlledMetadata.index_generation, 'generation-2026-07');
      assert.equal(controlledMetadata.canonical_id, 'mat-metadata');
      assert.equal(controlledMetadata.version_id, 'ver-metadata');
      assert.equal(controlledMetadata.source_hash, 'a'.repeat(64));
    }
    ...
    assert.equal(metadata.curriculum_year, 2024);
    for (const absentField of ['version', 'chapter', 'effective_from', 'effective_to'] as const) {
      assert.equal(Object.hasOwn(metadata, absentField), false);
    }
  });
});
