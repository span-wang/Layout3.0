...
> 版本：V1.1  
> 编制日期：2026-07-10  
> 当前状态：阶段 0 `PH3-13A`、阶段 1 `PH3-13B` 与阶段 2 检查点 `PH3-13C1` 已完成；`PH3-13C2` 待单独确认  
...
| 2 | `PH3-13C-single-file-vertical-slice-v1`（进行中：C1 已完成） | C1 已完成单文件受管接收和人工元数据；后续继续 DOCX/PDF 基础处理、pending 索引、快速质量、发布与回滚最小纵切 | C1 本地闭环已通过；阶段最终仍需一份测试资料验证 pending 不可检索、发布后可检索、回滚恢复旧版 | 8～12 人日 |
...
## 30. 推荐下一步（开始代码前必须再次确认）

阶段 0 `PH3-13A`、阶段 1 `PH3-13B` 与阶段 2 首个检查点 `PH3-13C1` 已完成：正式用户登记库已接入应用运行时，单个 DOCX/PDF 可完成受控接收、内容寻址受管复制、重复幂等、人工元数据确认和处理任务排队。下一步只提请确认 `PH3-13C2-basic-processing-pending-index-v1` 的实施边界，确认前不开始代码。

推荐的 `PH3-13C2` 只在 C1 受管原件和人工元数据基础上增加 DOCX/PDF 正文抽取、工件校验和定位映射，并通过 Main 专用适配层建立精确 pending 索引；快速质量、发布、回滚和生产检索切换继续留在后续检查点。正式边界仍需单独拆分并确认；确认前不连接生产资料、不处理当前 127 份冻结英语影子文档，也不切换现有默认检索链路。

`PH3-13C2` 的“本步纳入 / 本步明确不纳入 / 后续归属任务”必须先写回结构化进度源并由用户确认后，才能开始改代码。未来 RAGFlow 版本如通过 `document_ids + status` 排序前联合合同，可另行确认把 status 启用为额外防御；未通过时继续维持路线 A，不得把后置过滤或 metadata 单独过滤当作正式发布门禁。
