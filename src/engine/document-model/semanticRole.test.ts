...
test('PH2-07 已删除内置语义：旧文档里的题干语义恢复后应按无语义处理', async () => {
  ...
  assert.equal(normalizedDocument.blocks[0]?.semantic, undefined, '已删除的题干语义恢复后应被清掉。');
});
...
test('PH2-24 试卷答案位置：文末统一时顶层答案解析块应移动到末尾，隐藏时不参与正文', async () => {
  ...
  assert.deepEqual(
    ...
    ['content', 'content', 'answer', 'explanation'],
...
test('PH2-07 语义块样式绑定：导出 HTML 带语义标签和自定义颜色变量', async () => {
  ...
  assert(html.includes('>答案解析</span>'), '段落语义标签应输出真实文本，而不是依赖伪元素生成。');
});
