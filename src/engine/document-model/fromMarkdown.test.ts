...
test('PH2-07 语义块导入：role 前缀写入语义字段并保留正文格式', async () => {
  ...
  assert(html.includes('semantic-role-label semantic-role-label-inline'), '段落语义标签应输出块内胶囊标签元素。');
  assert(html.includes('>答案</span>'), '段落语义标签应输出真实文本，而不是依赖伪元素生成。');
});

test('PH2-07 已删除内置语义：role:题干 前缀不应再被识别或剥离', async () => {
  const document = await createLayoutDocumentFromMarkdown('role:题干 第一题');
  const paragraph = document.blocks[0];

  assert(paragraph, '应仍导入一个普通段落块。');
  assert.equal(paragraph.type, 'paragraph');
  assert.equal(paragraph.semantic, undefined, '已删除的题干语义不应再写入 semantic 字段。');
  assert.equal(getTextFromRuns(paragraph.textRuns), 'role:题干 第一题', '未识别的旧前缀应保留原文。');
});
